function [wellSols, states, schedulereport] = ...
      simulateScheduleAD_Modified(initState, model, schedule, varargin)
...
%}
    ...
    for i = 1:nSteps
        ...
        if model.BioGeo
            states{i,1}.Mineralogy = [];
            states{i,1}.Solution   = [];
            states{i,1}.IonMole    = [];
            states{i,1}.PHREEQC    = cell(model.G.cells.num,1);
            ...
            if model.parpool
                ...
                [model,states] = BioGeoModel_Parpool(model,states,i,dt);
            else
                [model,states] = BioGeoModel(model,states,i,dt);
            end
            %Applying Ion ransport if the model is 1D
            if (model.G.cartDims(1) > 1 && model.G.cartDims(2)==1 && model.G.cartDims(3) == 1)
                states = ExplicitIonTransport_1D(model,states,i,dt);
            end
            % if (model.G.cartDims(1)>1) && (model.G.cartDims(3) > 1)
            %      states = ExplicitIonTransport_2D(model,states,i,dt);
            % end
            % if (model.G.cartDims(1)>1) && (model.G.cartDims(2) > 1)
            %      states = ExplicitIonTransport_2D_Areal(model,states,i,dt);
            % end
            state.pressure   = states{i,1}.pressure;
            state.s          = states{i,1}.s;
            state.components = states{i,1}.components;
            state.Z_V        = states{i,1}.Z_V;
            state.Z_L        = states{i,1}.Z_L;
            state.L          = states{i,1}.L;
            state.x          = states{i,1}.x;
            state.y          = states{i,1}.y;
            state.FlowProps  = states{i,1}.FlowProps; 
        end
        if wantReport
            reports{i} = report;
        end
